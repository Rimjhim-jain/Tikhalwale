    ...
    <header className="fixed z-50 w-screen p-3 px-4 md:p-3 md:px-16 bg-primary">
      ...
      <div className='hidden md:flex w-full h-full items-center justify-between'>
        <Link to={"/"} className='flex items-center gap-2'>
          ...
          <p className='text-headingColor text-xl font-bold'>Tikhalwale</p>
        </Link>
        <div className='flex items-center gap-8'>
          ...
          <div className='relative flex items-center justify-center'>
            <MdShoppingCart className='text-textColor text-2xl cursor-pointer' onClick={showCart} />
            ...
            />
            {
              isMenu && (
                <motion.div
                  ...
                  {
                    user && user.email === "rimjhimsmile16@gmail.com" && (
                      <Link to={'/createItem'}>
            ...
            isMenu && (
              <motion.div
                ...
                <ul className=' flex flex-col '>
                  <li className='text-base text-headingColor hover:text-headingColor duration-100 transition-all cursor-pointer ease-in-out px-4 py-2 hover:bg-slate-100'
                  onClick={()=> setIsMenu(false)}
                  ><NavLink exact to='/*'>Home</NavLink></li>
                  <li className='text-base text-headingColor hover:text-headingColor duration-100 transition-all cursor-pointer ease-in-out px-4 py-2 hover:bg-slate-100'
                  onClick={()=> setIsMenu(false)}
                  ><NavLink exact to='/*'>Menu</NavLink></li>
                  <li className='text-base text-headingColor hover:text-headingColor duration-100 transition-all cursor-pointer ease-in-out px-4 py-2 hover:bg-slate-100'
                  onClick={()=> setIsMenu(false)}
                  ><NavLink exact to='/*'>About Us</NavLink></li>
                  <li className='text-base text-headingColor hover:text-headingColor duration-100 transition-all cursor-pointer ease-in-out px-4 py-2 hover:bg-slate-100'
                  onClick={()=> setIsMenu(false)}
                  ><NavLink exact to='/*'>Service</NavLink></li>
                </ul>

                <p className='m-2 py-2 rounded-md shadow-md flex items-center justify-center bg-gray-200 gap-3 cursor-pointer hover:bg-slate-300 transition-all duration-100 ease-in-out text-textColor text-base'
                  onClick={logout}
                >Logout <MdLogout /></p>
              </motion.div >
            )
